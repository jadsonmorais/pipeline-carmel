...
# TOKEN = config.get("INFRASPEAK_TOKEN")
...
def test_endpoint_filter(endpoint_path, filter_name, test_date):
    ...
    """
    # Limitamos a 1 apenas para validar a aceitação do parâmetro sem sobrecarregar a API
    ...
    try:
        ...
        if status_code == 200:
            ...
            # A API Infraspeak geralmente retorna 400 Bad Request se o parâmetro de busca não existir para a entidade
...
def main():
    # Definir uma data de teste (ex: 30 dias atrás)
    ...
    filtros_scheduled = [
        "date_min_updated_at",
        "date_min_start_date",
        "date_min_last_status_change_date"
    ]
    for filtro in filtros_scheduled:
