...
Fiscal CMERP API   ──────►  etls/fiscal/      ──────►  fiscal_raw_lancamentos
(HTTP idExp=80)              sync.py                    (PK = IDLANCAMENTOICMSBASE)
...
│   │
│   ├── fiscal/                  ← ETL API Fiscal CMERP (idExportacao=80)
│   │   ├── api.py               ← cliente HTTP (autenticação por headers)
...
# Fiscal CMERP — sync dos últimos 7 dias (padrão)
python -m etls.fiscal.sync

# Fiscal CMERP — período específico
python -m etls.fiscal.sync 2026-03-10 2026-03-16

# Fiscal CMERP — backfill histórico
...
| `carmel.fiscal_raw_lancamentos` | Fiscal CMERP | Lançamentos fiscais por empresa/hotel (PK = `IDLANCAMENTOICMSBASE`) |
...
- [skills/fiscal_etl.md](skills/fiscal_etl.md) — API CMERP, headers, parametros, chunking, comandos
